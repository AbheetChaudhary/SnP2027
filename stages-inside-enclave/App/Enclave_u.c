...
sgx_status_t ecall_openfhe_bfv_sample_code(sgx_enclave_id_t eid, int* retval)
{
	sgx_status_t status;
	ms_ecall_openfhe_bfv_sample_code_t ms;
	status = sgx_ecall(eid, 0, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t ecall_openfhe_bgv_sample_code(sgx_enclave_id_t eid, int* retval)
{
	sgx_status_t status;
	ms_ecall_openfhe_bgv_sample_code_t ms;
	status = sgx_ecall(eid, 1, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t ecall_openfhe_ckks_real_sample_code(sgx_enclave_id_t eid, int* retval)
{
	sgx_status_t status;
	ms_ecall_openfhe_ckks_real_sample_code_t ms;
